...
PNG createSpotlight(PNG image, int centerX, int centerY) {
  for(unsigned x = 0; x < image.width(); x++) {
    for(unsigned y = 0; y < image.height(); y++) {
      ...
      // decrease = distance / 2 / 100
      if(distance > 160)
        distance = 160;
      pixel.l = pixel.l *(1 - distance / 2 / 100);
    }
  }
  return image;
 ...
 * blue, based on if the pixel's hue value is closer to orgitange than blue.
 *
 * @param image A PNG object which holds the image data to be modified.
 *
...
PNG watermark(PNG firstImage, PNG secondImage) {
  ...
  for(unsigned x = 0; x < width; x++) {
    for(unsigned y = 0; y < height; y++) {
      HSLAPixel & mark = secondImage.getPixel(x,y);
      HSLAPixel & pixel = firstImage.getPixel(x,y);
      if(mark.l == 1 && pixel.l < .8)
        pixel.l = pixel.l + .2;
    }
  }
  return firstImage;
